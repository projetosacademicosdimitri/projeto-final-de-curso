...
<html>
    ...
    <title>SCA - Lista de pagamentos</title>
...
<div id="wrap">    
  ...
  <div class="panel-heading"><center><h4>Lista de Pagamentos</h4></center></div>
  ...
  <table class="table table-striped">
	...
	<td><b>Excluir</b></td>
       ...
       */ 
        ...
        $result = $obj->query("SELECT * FROM pagamento p INNER JOIN funcionario f ON p.idFunc = f.idFunc ORDER BY DataPgmto ASC, ValorLiquido ASC limit $init, $por_pagina");
	$result_total = $obj->query("SELECT count(*) as total FROM pagamento p INNER JOIN funcionario f ON p.idFunc = f.idFunc")->fetch(PDO::FETCH_OBJ);
        ...
        ?>
         ...
         <?php   foreach($result as $linha){  ?>
             <tr>
               ...
               <?php $data = implode("/",array_reverse(explode("-",$linha['DataPgmto']) ) );?>
                  
                <td><?php echo $data;?> </td>
                
                <td>R$ <?php echo number_format($linha['ValorLiquido'], 2, ',', '.'); ?></td>
                <td><a href="Pagamento.php?id=<?php echo $linha['idPgmto']; ?>&pagina=<?php echo $_GET['pagina'];?>  "><span title="Visualizar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-search"></span></a></td>
                <td><a href="UpdatePagamento.php?id=<?php echo $linha['idPgmto']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Editar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-edit"></a></td>
                <td><a onclick="return confirm('Deseja realmente excluir este registro?');" href="../../Controller/editePagamento.php?delete=<?php echo $linha['idPgmto']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Excluir" style="color: #ff0000; margin-left: 23%;" class="glyphicon glyphicon-remove"></a></td> 	
	     		</tr>
         <?php } ?> 
       </table>
</div>
...
<?php 
  ...
  if($ctrl > 1 ){
  echo '<li><a href="ListarPagamento.php?pagina=1" title="Primeira p&aacute;gina">&lt;&lt;</a></li>';
  echo '<li><a href="ListarPagamento.php?pagina='.($ctrl - 1).'" title="P&aacute;gina anterior">&lt;</a></li>';
  }
  if(($ctrl - 4) < 1)
  {
      $anterior = 1;
  ...
  for($i = $anterior ; $i <=$posterior && $i<= $num_paginas ;$i++){
          ...
          echo "<li><a href=\"ListarPagamento.php?pagina=$i\">$i</a></li>";
   

     }
	 if($ctrl < $num_paginas )
	 {
     echo '<li><a href="ListarPagamento.php?pagina='.($ctrl + 1).'" title="Pr&oacute;xima p&aacute;gina">&gt;</a></li>';
     echo "<li><a href=\"ListarPagamento.php?pagina=$num_paginas\" title=\"Ultima p&aacute;gina\">&gt;&gt;</a></li>";
	 }
     ?>
