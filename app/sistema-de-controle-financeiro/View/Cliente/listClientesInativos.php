...
<div id="wrap">    
  ...
  <table class="table table-striped">
	...
	<td><b>Ativar</b></td>
       ...
       */ 
        ...
        $result = $obj->query("SELECT * FROM cliente WHERE Status = 'Inativo' ORDER BY Nome ASC limit $init, $por_pagina");
	$result_total = $obj->query("SELECT count(*) as total FROM cliente WHERE Status = 'Inativo'")->fetch(PDO::FETCH_OBJ);
        ...
        ?>
           ...
           <?php   foreach($result as $linha){  ?>
             <tr>
                ...
                <td><a href="../../Controller/editeCliente.php?ativar=<?php echo $linha['idCliente']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Ativar" style="color: #00CD00; margin-left: 23%;" class="glyphicon glyphicon-off"></span></a></td> 	
	     			 </tr>
         <?php } ?> 
       </table>
</div>
  ...
  for($i = $anterior ; $i <=$posterior && $i<= $num_paginas ;$i++){
	 ...
	 if($ctrl < $num_paginas )
	 {
     echo '<li><a href="listClientesInativos.php?pagina='.($ctrl + 1).'" title="Pr&oacute;xima p&aacute;gina">&gt;</a></li>';
