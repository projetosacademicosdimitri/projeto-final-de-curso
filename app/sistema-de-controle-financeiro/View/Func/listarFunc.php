  ...
  <table class="table table-striped">
	...
	<td><b>Inativar</b></td>
       ...
       */ 
        ...
        $result = $obj->query("SELECT * FROM funcionario WHERE Status = 'Ativo' ORDER BY Nome ASC limit $init, $por_pagina");
	$result_total = $obj->query("SELECT count(*) as total FROM funcionario WHERE Status = 'Ativo'")->fetch(PDO::FETCH_OBJ);
...
<?php 
  ...
  for($i = $anterior ; $i <=$posterior && $i<= $num_paginas ;$i++){
	 ...
	 if($ctrl < $num_paginas )
	 {
     echo '<li><a href="listarFunc.php?pagina='.($ctrl + 1).'" title="Pr&oacute;xima p&aacute;gina">&gt;</a></li>';
