...
<html>
    ...
    <title>SCA - Lista de cheques</title>
...
<div id="wrap">    
  ...
  <div class="panel-heading"><center><h4>Lista de Cheques</h4></center></div>
  ...
  <table class="table table-striped">
	...
	<td><b>Excluir</b></td>
       ...
       */ 
        ...
        $result = $obj->query("SELECT * FROM cheque ORDER BY DataEmisao ASC limit $init, $por_pagina");
	$result_total = $obj->query("SELECT count(*) as total FROM cheque")->fetch(PDO::FETCH_OBJ);
        ...
        ?>
           ...
           <?php   foreach($result as $linha){  ?>
             <tr>
                ...
                <?php $data = implode("/",array_reverse(explode("-",$linha['DataEmisao']))); ?>

                <td><?php echo $data?> </td>
                <td><a href="cheque.php?id=<?php echo $linha['idCheque']; ?>&pagina=<?php echo $_GET['pagina'];?>  "><span title="Visualizar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-search"></span></a></td>
                <td><a href="updateCheque.php?id=<?php echo $linha['idCheque']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Editar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-edit"></span></a></td>
                <td><a onclick="return confirm('Deseja realmente excluir este registro?');" href="../../Controller/editeCheque.php?delete=<?php echo $linha['idCheque'];?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Excluir" style="color: #ff0000; margin-left: 23%;" class="glyphicon glyphicon-remove"></span></a></td> 	
	     			 </tr>
         <?php } ?> 
       </table>
</div>
...
<?php 
  ...
  if($ctrl > 1 ){
  echo '<li><a href="listCheque.php?pagina=1" title="Primeira p&aacute;gina">&lt;&lt;</a></li>';
  echo '<li><a href="listCheque.php?pagina='.($ctrl - 1).'" title="P&aacute;gina anterior">&lt;</a></li>';
  }
  if(($ctrl - 4) < 1)
  {
      $anterior = 1;
  ...
  for($i = $anterior ; $i <=$posterior && $i<= $num_paginas ;$i++){
          ...
          echo "<li><a href=\"listCheque.php?pagina=$i\">$i</a></li>";
   

     }
	 if($ctrl < $num_paginas )
	 {
     echo '<li><a href="listCheque.php?pagina='.($ctrl + 1).'" title="Pr&oacute;xima p&aacute;gina">&gt;</a></li>';
     echo "<li><a href=\"listCheque.php?pagina=$num_paginas\" title=\"Ultima p&aacute;gina\">&gt;&gt;</a></li>";
	 }
     ?>
