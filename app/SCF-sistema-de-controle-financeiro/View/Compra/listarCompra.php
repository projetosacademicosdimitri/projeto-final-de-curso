...
<html>
    ...
    <title>SCA - Lista de compras</title>
...
<div id="wrap">    
  ...
  <div class="panel-heading"><center><h4>Lista de Compras</h4></center></div>
  ...
  <table class="table table-striped">
	...
	<td><b>Excluir</b></td>
       ...
       */ 
        ...
        $result = $obj->query("SELECT * FROM compra INNER JOIN fornecedor f ON f.idFornecedor = compra.FornecedorId ORDER BY DataCompra ASC limit $init, $por_pagina");
	$result_total = $obj->query("SELECT count(*) as total FROM compra INNER JOIN fornecedor f ON f.idFornecedor = compra.FornecedorId")->fetch(PDO::FETCH_OBJ);
	...
	$num_paginas = ceil( $result_total->total /$por_pagina);
        ...
        ?>
           ...
           <?php   foreach($result as $linha){  ?>
             ...
             <?php $data = implode("/",array_reverse(explode("-",$linha['DataCompra'] )  ) ) ;?>
                <td><?php echo $data; ?> </td>
                

                <td><?php echo $linha['Quantidade'];?> </td>
                <td><a href="Compra.php?id=<?php echo $linha['idCompra']; ?>&pagina=<?php echo $_GET['pagina'];?>  "><span title="Visualizar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-search"></span></a></td>
                <td><a href="updateCompra.php?id=<?php echo $linha['idCompra']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Editar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-edit"></span></a></td>
                <td><a onclick="return confirm('Deseja realmente excluir este registro?');" href="../../Controller/editeCompra.php?delete=<?php echo $linha['idCompra']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Excluir" style="color: #ff0000; margin-left: 23%;" class="glyphicon glyphicon-remove"></a></td> 	
	     			 </tr>
         <?php } ?> 
       </table>
</div>
...
<?php 
  ...
  if($ctrl > 1 ){
  echo '<li><a href="listarCompra.php?pagina=1" title="Primeira p&aacute;gina">&lt;&lt;</a></li>';
  echo '<li><a href="listarCompra.php?pagina='.($ctrl - 1).'" title="P&aacute;gina anterior">&lt;</a></li>';
  }
  if(($ctrl - 4) < 1)
  {
      $anterior = 1;
  ...
  for($i = $anterior ; $i <=$posterior && $i<= $num_paginas ;$i++){
          ...
          echo "<li><a href=\"listarCompra.php?pagina=$i\">$i</a></li>";
   

     }
	 if($ctrl < $num_paginas )
	 {
     echo '<li><a href="listarCompra.php?pagina='.($ctrl + 1).'" title="Pr&oacute;xima p&aacute;gina">&gt;</a></li>';
     echo "<li><a href=\"listarCompra.php?pagina=$num_paginas\" title=\"Ultima p&aacute;gina\">&gt;&gt;</a></li>";
	 }
     ?>
