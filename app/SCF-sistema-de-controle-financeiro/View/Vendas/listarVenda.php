...
<html>
    ...
    <title>SCA - Lista de vendas</title>
  ...
  <body>
   ...
   <?php include '../MasterPages/menu1.php'; ?>
    ...
    <?php if(isset($_GET['ok'])){ ?>
     ...
     toastr.success("Venda atualizada.","Sucesso!"); //parametros: 1�mensagem,2� titulo
 
   <?php } ?>
   
   <?php if(isset($_GET['del'])){ ?>
   
     toastr.warning("Venda deletada."); //parametros: 1�mensagem,2� titulo
...
<div id="wrap">    
  ...
  <div class="panel-heading"><center><h4>Lista de Vendas</h4></center></div>
  ...
  <table class="table table-striped">
	...
	<td><b>Excluir</b></td>
       ...
       */ 
        ...
        $result = $obj->query("SELECT * FROM venda INNER JOIN sacoplastico ON sacoplastico.IdSaco = venda.IdSaco INNER JOIN cliente ON cliente.idCliente = venda.ClienteId ORDER BY venda.DataVenda ASC limit $init, $por_pagina");
	$result_total = $obj->query("SELECT count(*) as total FROM venda INNER JOIN sacoplastico ON sacoplastico.IdSaco = venda.IdSaco INNER JOIN cliente ON cliente.idCliente = venda.ClienteId")->fetch(PDO::FETCH_OBJ);
        ...
        ?>
         ...
         <?php   foreach($result as $linha){  ?>
             ...
             <td> <?php echo $linha['Nome'];?></td>
                    <?php $data = implode("/",array_reverse(explode("-",$linha['DataVenda']) ) );?>
                  <td><?php echo $data; ?></td>
               
                <td><?php echo $linha['Quantidade'];?> </td>
                <td><a href="Venda.php?id=<?php echo $linha['idVenda']; ?>&pagina=<?php echo $_GET['pagina'];?>  "><span title="Visualizar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-search" title="Visualizar"></span></a></td>
                <td><a href="UpdateVenda.php?id=<?php echo $linha['idVenda']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Editar" style="color: #000000; margin-left: 23%;" class="glyphicon glyphicon-edit" title="Editar"></a></td>
                <td><a onclick="return confirm('Deseja realmente excluir este registro?');" href="../../Controller/editeVenda.php?delete=<?php echo $linha['idVenda']; ?>&pagina=<?php echo $_GET['pagina'];?>"><span title="Excluir" style="color: #ff0000; margin-left: 23%;" class="glyphicon glyphicon-remove" title="Excluir"></a></td> 	
	     			 </tr>
         <?php } ?> 
       </table>
</div>
...
<?php 
  ...
  if($ctrl > 1 ){
  echo '<li><a href="listarVenda.php?pagina=1" title="Primeira p&aacute;gina">&lt;&lt;</a></li>';
  echo '<li><a href="listarVenda.php?pagina='.($ctrl - 1).'" title="P&aacute;gina anterior">&lt;</a></li>';
  }
  if(($ctrl - 4) < 1)
  {
      $anterior = 1;
  ...
  for($i = $anterior ; $i <=$posterior && $i<= $num_paginas ;$i++){
          ...
          echo "<li><a href=\"listarVenda.php?pagina=$i\">$i</a></li>";
    
     }
	 if($ctrl < $num_paginas ){
     echo '<li><a href="listarVenda.php?pagina='.($ctrl + 1).'" title="Pr&oacute;xima p&aacute;gina">&gt;</a></li>';
	 echo "<li><a href=\"listarVenda.php?pagina=$num_paginas\" title=\"Ultima p&aacute;gina\">&gt;&gt;</a></li>";}
     ?>
